document.body.innerHTML +=`<style>
		...
		setTimeout(function() {
			...
			var startBw = function() {
				if (game.global.mapsActive) {
					if (document.getElementById("togglerepeatUntil") 
						&& document.getElementById("togglerepeatUntil").offsetParent !== null 
						&& document.getElementById("togglerepeatUntil").innerHTML.indexOf("Repeat for Items") == -1) {
						document.getElementById("togglerepeatUntil").click();
				console.log('3');
					}
					setTimeout(startBw, 1000);
					return;
				}
				
				
				if (document.getElementById("mapsBtnText").innerHTML.indexOf("Abandon Soldiers") > - 1)
					mapsClicked();
				...
				if (document.getElementById("mapsBtnText") 
					...
					if (map != null) {
						if (map.name === bwStopMap) {
							lastMap = map.level - 15;
							waitForMap();
						} else {
							selectMap(map.id);
							runMap();
			...
			var createItemsMap = function() {
				if (game.global.world == bwStartZone) {
					startBw();
					return;
				}
			...
			var create20Map = function() {
				...
				console.log(getPortalTime() + " zone " + game.global.world + ",  map created, fragments " + fragmentsCount + " - " + game.resources.fragments.owned + ", fluffy exp rate " + getFluffyExperienceRate());
				setTimeout(function() {
					startMap();
				}, 50);
			}
			...
			var itemsAction = function() {
				...
				if (findZoneNumber() == bwStartZone && game.global.lastClearedCell > 85) {
					actionNeeded = true;
				}
	...
	var startPerkUpgrades = function() {
		if (game.global.world > 450 && !perksSet && autoPerk && game.global.world < 590) {
			perksSet = true;
			viewPortalUpgrades();
			if (autoPerkNew) {
...
var logFluffyExpInterval = setInterval(function() {
	...
	if (lastFluffyExpLog < game.global.world) {
		...
		console.log(getPortalTime() + " " + game.global.world + " zone, fluffy exp rate: " + getFluffyExperienceRate() + " exp / s");
	}
}, 3000);
...
var initialAbandons = [525, 540, 554, 555, 564, 568, 585, 603, 615, 626, 636, 638, 645, 653];
var abandons = initialAbandons.slice();
var noActionZones = [570, 571, 572, 573, 574, 577, 578, 579, 580, 581, 582, 583, 584, 585, 586, 587, 588, 589, 592, 593, 594, 595, 596, 597, 598, 601, 602, 603, 604, 607, 608, 609, 610, 611, 612, 613, 614, 616, 617, 618, 619, 622, 623, 624, 639, 641, 642, 643, 662, 663, 664, 665, 666];
for (var iii = 0; iii < 599; iii++) { if (iii != 585) noActionZones.push(iii); }
...
forcedPortalZone = 681;
actionZones = 60 + 30;
initialRestartZone = 645 + 30;
restartOnZone = 645 + 30;
var fastFightZones = restartOnZone;

gaSecsZones = -100;
var stopGaAssistantAfter600Spire = true;
var autoPerkNew = true;
var autoPerkNewMode = "Power_II";
...
var windFormationZones = "|601|602|603|604|605|616|617|618|619|620|631|632|633|634|635|646|647|648|649|650|661|662|663|664|665|676|677|678|679|680|"
...
var bwStartZone = 660;
var bwStopMap = "Bionic Wonderland XXXIX";
