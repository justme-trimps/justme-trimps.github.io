...
var challengeToTry = 'Quagmire'; 
//var challengeToTry = 'Bublé'; 
var minMeltingZone = 59;
var autoCancelActive = false;
var smithiesWanted = 16;
var minPrismaticPalaceZone = 72;
...
var plusZeroZones = [85, 88, 94, 95, 96];
var plusOneZones = [/*54, 60, 61, 64, 80, 81, 82, 83, 84 /*, 90, 91, 92, 93, 94*//* 20% more dmg */];
var plusTwoZones = [89, 90, 91, 92, 93];//61, 63/*69, 70*/];
var plusThreeZones = [78];
var plusFourZones = [57, 68, 69];
var plusFiveZones = [66];
var plusSixZones = [];

var blackBogs = [];

var initBlackBogs = function() {
	for (var i = 0; i <= 150; i++) blackBogs[i] = -1;
	blackBogs[9] = 1;
	blackBogs[10] = 1;
	blackBogs[11] = 1;
	blackBogs[12] = 1;
...
var forcedPortalWorld = 97;
var mapMode = "lsc";
var maxBlackBogZone = 49;
...
var changeAutobuyingNumbersInterval = setInterval(function() {
	if (changeAutoBuy && !game.global.mapsActive) {
		...
		if (game.global.world >= 85) { autobuyingEquipmentNumber = 55; autobuyingArmNumber = 55; }
		if (game.global.world >= 89) { autobuyingEquipmentNumber = 40; autobuyingArmNumber = 40; buyShields = true; }
		if (game.global.world >= 94) { autobuyingEquipmentNumber = 63; autobuyingArmNumber = 63; }
	}
}, 1000 * 1);
...
var jestimpInterval = setInterval(function() { 
	...
	if ((game.global.challengeActive + "") === "Quagmire" && document.getElementById('worldName') && document.getElementById('worldName').innerText !== "The Black Bog")
		return;
	...
	if (game.global.world >= minJestimpZone || minJestimpZone == -1) {
		...
		if (jestimpMode && badGuyNameString && badGuyNameString.toLowerCase().indexOf("jestimp") == -1) {
			if (pendingLogs.Loot.length > 0 && pendingLogs.all[pendingLogs.Loot[0]].toLowerCase().indexOf('jestimp') > -1) {
				return;
			}
			...
			if (logMessages.length) {
				//console.log(JSON.stringify(pendingLogs));
...
var buyThingsInterval = setInterval(function() {
	...
	if (document.getElementById('Tribute')) if (buyTributes) buyBuilding('Tribute');
...
var hireFarmersInterval = setInterval(function() {
	...
	if (game.global.world < 70) {
		...
		if (game.global.world == 70 && game.global.lastClearedCell <= 88) {
			setMax(0.25, false);
			numTab(6);
			cancelTooltip();
			buyJob('Farmer');
...
var shouldFightSomeMap = function() {
	...
	if (!game.global.mapsActive && game.buildings.Smithy.owned == smithiesWanted && game.global.world > 70) {
		for (var i = game.global.mapsOwnedArray.length - 1; i > -1; i--) {
	...
	if (!ignorePrismaticPalace) {
		if (startMapsWorld <= game.global.world && !game.global.mapsActive && game.global.world >= minPrismaticPalaceZone && game.global.lastClearedCell > 78) {
			for (var i = game.global.mapsOwnedArray.length - 1; i > -1; i--) {
	...
	if (blackBogs[game.global.world] == 1 && game.global.lastClearedCell > 78) {
		blackBogs[game.global.world] = 0;
		return true;
	}
...
var repeatMaps = setInterval(function() {
	if (shouldFightSomeMap()) {
		...
		if (game.buildings.Smithy.owned == smithiesWanted && game.global.world > 70) {
			for (var i = game.global.mapsOwnedArray.length - 1; i > -1; i--) {
				if (game.global.mapsOwnedArray[i].name == "Melting Point") {
					var button = document.getElementById(game.global.mapsOwnedArray[i].id);
					if (button && button.getAttribute("class").indexOf("noRecycleDone") == -1) {
		...
		if (!ignorePrismaticPalace) {
			for (var i = game.global.mapsOwnedArray.length - 1; i > -1; i--) {
				if (game.global.mapsOwnedArray[i].name == "Prismatic Palace" && game.global.world >= minPrismaticPalaceZone && game.global.lastClearedCell > 78) {
					var button = document.getElementById(game.global.mapsOwnedArray[i].id);
					if (button && button.getAttribute("class").indexOf("noRecycleDone") == -1) {
						button.click();
						specialZoneRun = true;
		...
		if (!specialZoneRun && game.global.lastClearedCell > 78) {
			if (blackBogs[game.global.world] == 0) {
				for (var i = game.global.mapsOwnedArray.length - 1; i > -1; i--) {
					if (game.global.mapsOwnedArray[i].name == "The Black Bog") {
						var mapButton = document.getElementById(game.global.mapsOwnedArray[i].id);
						if (mapButton && mapButton.getAttribute("class").indexOf("noRecycleDone") == -1) {
							mapButton.click();
							specialZoneRun = true;
							blackBogs[game.global.world] = -1;
						}
					}
				}
			}
...
var restartMapInterval = setInterval(function() { 
	if (game.global.world == 70 
		...
		&& game.global.lastClearedMapCell > 130 
		&& document.getElementById('worldName') 
		&& document.getElementById('worldName').innerText === "The Black Bog"
		&& game.global.mapsActive) {
		mapsClicked(true); 
		cancelTooltip();
		runMap();
...
function setSaveCookie() {
	setCookie('cookie-optimizer2', game.global.totalRadPortals + '###' + game.buildings.Tribute.owned + '###'+ game.global.totalVoidMaps + '###' + save(true), 10);
}
...
var shouldLoadOptimizedSave = function(reset, tributes, voidMaps, debug) {
	...
	if (voidMaps == game.global.totalVoidMaps && tributes > game.buildings.Tribute.owned) {
		if (debug) {
			console.log('shouldLoadOptimizedSave save has more tributes ' + tributes + " " + game.buildings.Tribute.owned);
		}
		return true;
	}
...
var shouldSaveOptimizedSave = function(reset, tributes, voidMaps, debug) {
	...
	if (voidMaps == game.global.totalVoidMaps && tributes < game.buildings.Tribute.owned) {
		if (debug) {
			console.log('shouldSaveOptimizedSave true: more tributes than save ' + tributes + " " + game.buildings.Tribute.owned);
		}
		return true;
	}
...
var cookieOptimizerInterval = setInterval(function() { 
	...
	var cookieValue = getCookie('cookie-optimizer2');
