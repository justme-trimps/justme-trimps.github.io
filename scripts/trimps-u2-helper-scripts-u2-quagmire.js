...
var plusOneZones = [54, 60, 61, 64, 80, 81, 82, 83, 84];
var plusTwoZones = [69, 70];
var plusThreeZones = [];
var plusFourZones = [];
var plusFiveZones = [];
var plusSixZones = [];
...
var voidMapZone = 70;
var maxVoidMapZone = 75;

var abandonChallengeZone = 74;
var forcedPortalWorld = 85;
var mapMode = "lsc";
...
var changeJestimpTargetToWoodInterval = setInterval(function() {
	if (smithiesWanted > -1 && game.global.world >= minMeltingZone && !wentForSmithy) {
		if (smithiesWanted > game.buildings.Smithy.owned) {
			if (game.resources.metal.owned > 2.5e+24) {
				fireMode();
				setMax(1, false);
				numTab(6);
				cancelTooltip();
...
var changeAutobuyingNumbersInterval = setInterval(function() {
	if (changeAutoBuy && !game.global.mapsActive) {
		...
		if (game.global.world >= 80) { autobuyingEquipmentNumber = 20; autobuyingArmNumber = 20; }
	}
}, 1000 * 1);
...
var hireFarmersInterval = setInterval(function() {
	...
	if (game.global.world < 70) {
		...
		if (game.global.totalVoidMaps > 3) {
			setMax(0.25, false);
			numTab(6);
			cancelTooltip();
			buyJob('Farmer');
...
var shouldFightSomeMap = function() {
	...
	if (!game.global.mapsActive && voidMapZone != -1 && (game.global.world >= voidMapZone && game.global.world <= maxVoidMapZone) && game.global.lastClearedCell > 78) {
	...
	if (!game.global.mapsActive && game.buildings.Smithy.owned == smithiesWanted) {
		for (var i = game.global.mapsOwnedArray.length - 1; i > -1; i--) {
	...
	if (getStackCount('quagmireExhaustedStacks') > 8 && game.global.lastClearedCell > 78) {
		if (game.global.world <= maxBlackBogZone) {
			return true;
		}
	}
...
var repeatMaps = setInterval(function() {
	if (shouldFightSomeMap()) {
		...
		if (!specialZoneRun && getStackCount('quagmireExhaustedStacks') > 8 && game.global.lastClearedCell > 78) {
			if (game.global.world <= maxBlackBogZone) {
				for (var i = game.global.mapsOwnedArray.length - 1; i > -1; i--) {
