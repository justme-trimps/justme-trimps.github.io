...
var fightMayhemStacksInterval;

var shouldLoadFightMayhemStacksSave = function(save) {
	...
	if (game.challenges.Mayhem.stacks > save.mayhemStacks + stacksDifference - 1)
		return true;

	return false;
};
var shouldSaveFightMayhemStacksSave = function(save) {
	if (!game.global.mapsActive)
		return false;

	if (save == null || save.cell < (game.global.world * 100 + game.global.lastClearedCell))
		return true;

	if (game.portal.Frenzy.frenzyLeft() <= 0)
		return false;

	if (game.challenges.Mayhem.stacks < save.mayhemStacks)
		return true;

	return false;
};

fightMayhemStacksInterval = setSomeInterval(fightMayhemStacksInterval, shouldSaveFightMayhemStacksSave, shouldLoadFightMayhemStacksSave, 1000);
