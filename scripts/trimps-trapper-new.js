	...
	setTimeout(function() {
		...
		setTimeout(function() {
			...
			var createItemsMap = function() {
				...
				setTimeout(function() {
					if (newMapNeeded) {
						console.log(getPortalTime() + " zone " + game.global.world +", fragments " + game.resources.fragments.owned);
			...
			var create20Map = function() {
				...
				console.log(getPortalTime() + " map created, fragments " + game.resources.fragments.owned);
				setTimeout(function() {
					startMap();
				}, 50);
			}
			...
			var itemsAction = function() {
				...
				if (findZoneNumber() == 520 && game.global.lastClearedCell > 92) {
					actionNeeded = true;
				}
				if (findZoneNumber() == 535 && game.global.lastClearedCell > 92) {
					actionNeeded = true;
				}
				if (findZoneNumber() == 545 && game.global.lastClearedCell > 92) {
					actionNeeded = true;
				}
				if (findZoneNumber() == 550 && game.global.lastClearedCell > 92) {
					actionNeeded = true;
				}
		...
		trimpsIntervals.push(setInterval(function() {
					...
					if (timerId) {
						...
						if (timer) {
							...
							if (innerHTML.match(/^[0-9]+ Sec(s)?$/) 
								...
								|| innerHTML.match(/^[0-9]{1,4} Year(s)? [0-9]+ Day(s)?$/) 
			...
			trimpsIntervals.push(setInterval(function() { checkStorage("Barn", "foodTimeToFill", "food"); }, 500));
			trimpsIntervals.push(setInterval(function() { checkStorage("Shed", "woodTimeToFill", "wood"); }, 500));
			trimpsIntervals.push(setInterval(function() { checkStorage("Forge", "metalTimeToFill", "metal"); }, 500));
...
trimpsIntervals.push(setInterval(function() {
	var goodModsNames = "FluffyExp|MinerSpeed|empty|metalDrop|ExplorerSpeed|fragmentsDrop|trimpHealth|critChance|trimpAttack|voidMaps|critDamage|plaguebringer";
...
autoExitSpireCells = 999;
var autoExitSpire = false;

// 2018.07.31 fluffyMatureInterval

var initialAbandons = [525, 540, 554, 555/*, 516, 520/*502, 505*/];
var abandons = initialAbandons.slice();
var noActionZones = [498, 501, 502, 503, 504, 505, 506, 507, 508, 509, 510, 511, 512, 513, 514, 517, 518, 519, 522, 523, 524, 526, 527, 528, 529, 530, 531, 532, 533, 534, 541, 542, 543, 544, 547, 548, 549];
var repeatForAnyZones = [500, 540, 554];
var autoNurseryInterval = 1200
var forcedPortalZone = 547 + 15;
var actionZones = 41 + 15;
var fastFight = true;
var fastFightZones = 300;
var lastZoneEquipment =  190;
var autoEquipNumber = 190;
var initialRestartZone = 540 + 15;
var restartOnZone = 540 + 15;
var nurseryZones = 50 + 15;
