...
setTimeout(function() {
	...
	setTimeout(function() {
		...
		var startMap = function() { 
			...
			setTimeout(function() { 
				...
				setTimeout(function() { 
					...
					if (game.global.world == 495 && trySpire500) {
						return;
					}
					waitForMap();
				}, 1000);
		...
		var abandonNeeded = function() {
			if (game.global.world == 495 && game.global.lastClearedCell < 63) {
				return false;
			}
		...
		var createItemsMap = function() {
			if (!voidMapsFinished 
				...
				&& game.global.lastClearedCell > 87 
				&& !abandonForced 
			...
			if (game.global.world == 476) {
				newMapNeeded = true;
				console.log(getPortalTime() + " zone 476, fragments " + game.resources.fragments.owned);
				document.getElementById("biomeAdvMapsSelect").value = "Random";
				document.getElementById("advExtraLevelSelect").value = "6";
				if (game.resources.fragments.owned > 3.08e+43) {
					document.getElementById("advSpecialSelect").value = 0;
				} else if (game.resources.fragments.owned > 0.82e+43) {
					document.getElementById("advSpecialSelect").value = 0;
				...
				updateMapNumbers();
				updateMapCost();
			} else if (game.global.world == 494) {
				newMapNeeded = true;
				console.log(getPortalTime() + " zone 494, fragments " + game.resources.fragments.owned);
				if (game.resources.fragments.owned > 0.242e+45) {
					document.getElementById("biomeAdvMapsSelect").value = "Random";
					document.getElementById("lootAdvMapsRange").value = 0;
					document.getElementById("difficultyAdvMapsRange").value = 0;
			...
			} else if (game.global.world == 495) {
				newMapNeeded = true;
				console.log(getPortalTime() + " zone 495, fragments " + game.resources.fragments.owned);
/*				if (game.resources.fragments.owned > 1.71e+45) {
					if (game.resources.fragments.owned > 2.29e+45) {
						document.getElementById("sizeAdvMapsRange").value = "3";
					} else if (game.resources.fragments.owned > 2.08e+45) {
						document.getElementById("sizeAdvMapsRange").value = "2";
					} else if (game.resources.fragments.owned > 1.88e+45) {
...
MAREK */
				if (game.resources.fragments.owned > 0.451e+45) {
					if (game.resources.fragments.owned > 1.1e+45) {
						document.getElementById("sizeAdvMapsRange").value = "9";
					} else if (game.resources.fragments.owned > 0.989e+45) {
						document.getElementById("sizeAdvMapsRange").value = "8";
					} else if (game.resources.fragments.owned > 0.814e+45) {
						document.getElementById("sizeAdvMapsRange").value = "6";
					} else if (game.resources.fragments.owned > 0.606+45) {
						document.getElementById("sizeAdvMapsRange").value = "3";
					} else {
						document.getElementById("sizeAdvMapsRange").value = "0";
					}
			...
			} else if (game.global.world == 500) {
				newMapNeeded = true;
				document.getElementById("advExtraLevelSelect").value = "5";
				document.getElementById("biomeAdvMapsSelect").value = "Random";
				document.getElementById("advSpecialSelect").value = 0;

				if (game.resources.fragments.owned >= 1.33e+45) {
					document.getElementById("biomeAdvMapsSelect").value = "Plentiful";
				}

				if (game.resources.fragments.owned >= 14.5e+45) {
					document.getElementById("advSpecialSelect").value = "lmc";
				}

				updateMapNumbers();
				updateMapCost();
			} else if (game.global.world == 506) {
				newMapNeeded = true;
				document.getElementById("biomeAdvMapsSelect").value = "Random";
				document.getElementById("advExtraLevelSelect").value = "6";
				if (game.resources.fragments.owned > 28.4e+45) {
					// perfect sliders
				} else if (game.resources.fragments.owned > 5.3e+45) {
					document.getElementById("lootAdvMapsRange").value = 0;
				} else {
					document.getElementById("lootAdvMapsRange").value = 0;
					document.getElementById("difficultyAdvMapsRange").value = 0;
				...
				console.log(getPortalTime() + " zone 516, fragments " + game.resources.fragments.owned);
				newMapNeeded = true;
				document.getElementById("biomeAdvMapsSelect").value = "Random";
				document.getElementById("lootAdvMapsRange").value = 0;
				document.getElementById("advExtraLevelSelect").value = "6";
				document.getElementById("advSpecialSelect").value = 0;
				if (game.resources.fragments.owned > 3.33e46) {
					document.getElementById("advSpecialSelect").value = "p";
					// MAREK document.getElementById("advExtraLevelSelect").value = "7";
				}
				updateMapNumbers();
				updateMapCost();
				...
				console.log(getPortalTime() + " zone 525, fragments " + game.resources.fragments.owned);
				newMapNeeded = true;
				...
				if (testPlus9 && game.resources.fragments.owned > 4.23+47) {
					document.getElementById("advExtraLevelSelect").value = "9";
					document.getElementById("sizeAdvMapsRange").value = "0";
				} else {
					if (game.resources.fragments.owned > 2.72e+47) {
						// MAREK document.getElementById("advSpecialSelect").value = "p";
						document.getElementById("advExtraLevelSelect").value = "8";
					}

					if (game.resources.fragments.owned > 4.03e+47) {
						document.getElementById("difficultyAdvMapsRange").value = 4;
					}

					if (game.resources.fragments.owned > 4.9e+47) {
		...
		var itemsAction = function() {
			...
			if (findZoneNumber() == 475 && game.global.lastClearedCell > 85) {
				actionNeeded = true;
			}
			if (findZoneNumber() == 485 && game.global.lastClearedCell > 85) {
				actionNeeded = true;
			}
			if (findZoneNumber() == 490 && game.global.lastClearedCell > 85) {
				actionNeeded = true;
			}
			
			if (findZoneNumber() == 505 && game.global.lastClearedCell > 85) {
				actionNeeded = true;
			}
			
			if (findZoneNumber() == 520 && game.global.lastClearedCell > 85) {
				actionNeeded = true;
			}
			
			if (findZoneNumber() == 535 && game.global.lastClearedCell > 85) {
				actionNeeded = true;
			}
			...
			if (actionNeeded){
				...
				setTimeout(function() { itemsAction(); }, 1500)
			}
		}
				...
				if (timerId) {
					...
					if (timer) {
						if (timer.innerHTML.match(/^[0-9]+ Sec(s)?$/) 
							|| timer.innerHTML.match(/^[0-9]+ Min(s)? [0-9]+ Sec(s)$/) 
							|| timer.innerHTML.match(/^[0-9]([0-9])? Year(s)? [0-9]+ Day(s)$/) 
							|| timer.innerHTML.match(/^[0-9][0-9][0-9] Years [0-9]+ Day(s)$/) 
							|| timer.innerHTML.match(/^[0-9]+ Day(s)? [0-9]+ Hour(s)$/) 
							|| timer.innerHTML.match(/^[0-9]+ Hour(s)? [0-9]+ Min(s)$/)) {
							click = true;
						}
					}
				}
...
var autoExitSpireCells = 20;
var lastMap = 0;
var autoPauseGenerator = true;
var autoPauseGeneratorZone = restartOnZone - 3;
var buyEquipmentStartZone = 0;
var buyArmStartZone = 0;
var noActionZones = [];
var repeatForAnyZones = [];
...
document.getElementById("fightBtn").click();
	...
	initialRestartZone = 525;
	restartOnZone = 525;
	initialAbandons = [/*494,*/ 495, 510/*, 516, 520/*502, 505*/];
	noActionZones = [496, 501, 502, 503, 504, 511, 512, 513, 514, 522, 523, 524, 540];
	abandons = initialAbandons.slice();
	gaSecsZones = 15 + 15;
	actionZones = 16 + 15; //7 // 10; //35;
	gymNumber = 2200;
	miZones = -100;//160;//-100;

	autoEquipNumber = 200;
    lastZoneEquipment = autoEquipNumber;
	minHeliumPerHourPercent = 0.95;
	autoExitSpire = true;

	forcedPortalZone = 529;
	...
	///////////////////////
	autoExitSpireCells = 78; // MAREK 78
	autoNurseryInterval = 1000
	nurseryZones = 20 + 15;
	autoPauseGeneratorZone = 480
	//var tmpInterval = setInterval ( function() { if (game.global.world == 500 && game.global.spireActive) console.log(game.global.lastClearedCell); }, 1000);
	repeatForAnyZones = [495, 500, 510];

	var testPlus9 = false;
	var autohireDisabled = false;
	var trySpire500 = false;

// 2018 05 17 testPlus9
...
if (gogogo) {
	var autoGoldenVoid = false;
	var autoGoldenHelium = false;
	var autoGoldenBattle = true;
	initialRestartZone = 650;
	restartOnZone = 650;
	autoFormation = false;
	autoPauseGeneratorZone = restartOnZone - 20;
	abandons = [495];
	actionZones = restartOnZone - 494;
	gaSecsZones = -1000;
	miZones = -100;
	autoExitSpire = false;
	nurseryZones = 0;
	trySpire500 = true;
	forcedPortalZone = 1000;
	for (var i = 530; i < 650; i++) repeatForAnyZones.push(i)
	perksSet = true;
	gymNumber = 2200;
}
/*
...
setInterval(function() {
	if (game.global.world == 599 && game.global.lastClearedCell > 90) clearAllIntervals();
}, 5000)
);
*/