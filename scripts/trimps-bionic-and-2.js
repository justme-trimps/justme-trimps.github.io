...
setTimeout(function() {
	...
	var waitForMap = function() { 
		...
		setTimeout(function() { waitForMap(); }, 3000)
	}
	...
	var createItemsMap = function() { 
		if (document.getElementById("selectMapBtn") && document.getElementById("selectMapBtn").innerHTML.indexOf("Continue") > -1 && document.getElementById("recycleMapBtn")) {
	...
	var action = function() {
		if (findVoidMapsNumber() > 0 && findZoneNumber() >= restartOnZone && restartOnZone === lastItems) {
			console.log("zone " + findZoneNumber());
			document.getElementById("mapsBtn").click();
			console.log("maps clicked");
			clearInterval(interval);
			setTimeout(function() { abandonMaps(findVoidMap); }, 1000);
		} else {
	...
	var itemsAction = function() {
		if ((findZoneNumber() >= itemsStart && findZoneNumber() % 10 === 5 && findZoneNumber() > lastItems) || (findZoneNumber() == restartOnZone)){
			console.log("itemsAction " + findZoneNumber());
			document.getElementById("mapsBtn").click();
			console.log("maps clicked");
			lastItems = findZoneNumber();
			setTimeout(function() { abandonMaps(createItemsMap); }, 1000);
		} else {
			console.log("no need to buy items in " + findZoneNumber() + " zone");
			setTimeout(function() { itemsAction(); }, 3000)
		}
	}
...
setInterval(function() { 
	...
	if (document.getElementById("metalCollectBtn") && document.getElementById("metalCollectBtn").offsetParent !== null) {
