...
var forcedPortalWorld = 101;

var plusZeroZones = [71, 73, 75, 77, 84, 85, 91, 92, 93, 94, 95, 96, 97]
var plusOneZones = [/*54, 60, 61, 64, 80, 81, 82, 83, 84 /*, 90, 91, 92, 93, 94*//* 20% more dmg */];
var plusTwoZones = [79, 80, 83];//[89, 90, 91, 92, 93];//61, 63/*69, 70*/];
var plusThreeZones = [55, 98, 99];
var plusFourZones = [];
...
var changeAutobuyingNumbersInterval = setInterval(function() {
	if (changeAutoBuy && !game.global.mapsActive) {
		...
		if (game.global.world >= 94) { autobuyingEquipmentNumber = 40; autobuyingArmNumber = 40; }
		if (game.global.world >= 95) { autobuyingEquipmentNumber = 30; autobuyingArmNumber = 30; }
	}
}, 1000 * 1);
...
var hireFarmersInterval = setInterval(function() {
	...
	if (game.global.world < 80) {
		...
		if (game.global.world == 80 && game.global.lastClearedCell <= 88) {
			setMax(0.25, false);
			numTab(6);
			cancelTooltip();
			buyJob('Farmer');
